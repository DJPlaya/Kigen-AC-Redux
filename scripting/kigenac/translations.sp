...
Trans_OnPluginStart()
{
	...
	SetTrieString(f_hTemp, KAC_GBANNED, "You are banned from all Kigen's Anti-Cheat Redux (KACR) protected servers. See https://djplaya.github.io/kigen-ac_redux for more information");
	SetTrieString(f_hTemp, KAC_VACBANNED, "This Kigen's Anti-Cheat Redux (KACR) protected server does not allow VALVe's Anti-Cheat (VAC) banned players");
	...
	SetTrieString(f_hTemp, KAC_CLIENTCORRUPT, "Your client has become corrupted or is misconfigured. Please restart your game before reconnecting");
	...
	SetTrieString(f_hTemp, KAC_UNABLETOCONTACT, "Unable to contact the KACR Master");
	...
	SetTrieString(f_hTemp, KAC_AUTOASSIGNED, "[KACR] You have been Auto-Assigned to a team");
	SetTrieString(f_hTemp, KAC_SAYBLOCK, "[KACR] Your say has been blocked due to a invalid character");
	SetTrieString(f_hTemp, KAC_FORCEDREVAL, "[KACR] Forced revalidation on all connected players");
	SetTrieString(f_hTemp, KAC_CANNOTREVAL, "[KACR] Cannot force revalidation until all player have already been validated");
	...
	SetTrieString(f_hTemp, KAC_GBANNED, "Vous avez été banni de tous les serveurs protégés par Kigen's Anti-Cheat Redux (KACR). Voir https://djplaya.github.io/kigen-ac_redux pour plus d'informations");
	SetTrieString(f_hTemp, KAC_VACBANNED, "Ce serveur protégé par Kigen's Anti-Cheat Redux (KACR) n'autorise pas les joueurs bannis par VALVe's Anti-Cheat (VAC)");
	...
	SetTrieString(f_hTemp, KAC_MUTED, "%N a été rendu silencieux par Kigen's Anti-Cheat Redux");
	...
	SetTrieString(f_hTemp, KAC_NETMOD, "Network");
	SetTrieString(f_hTemp, KAC_UNABLETOCONTACT, "Impossible de contacter le server maître KACR");
	SetTrieString(f_hTemp, KAC_EYEMOD, "Eye Test");
	SetTrieString(f_hTemp, KAC_ANTIWH, "Anti-Wallhack");
	...
	SetTrieString(f_hTemp, KAC_AUTOASSIGNED, "[KACR] Vous avez rejoint automatiquement une team");
	SetTrieString(f_hTemp, KAC_SAYBLOCK, "[KACR] Vous ne pouvez plus écrire dû à un caractère non autorisé");
	SetTrieString(f_hTemp, KAC_FORCEDREVAL, "[KACR] Revalidation forcée sur tous les joueurs connectés");
	SetTrieString(f_hTemp, KAC_CANNOTREVAL, "[KACR] Revalidation impossible, tous les joueurs ont déjà été validés");
	...
	SetTrieString(f_hTemp, KAC_GBANNED, "Sei bannato da tutti i server protetti dall'anticheats Kigen Redux (KACR). Visita https://djplaya.github.io/kigen-ac_redux per ulteriori informazioni");
	SetTrieString(f_hTemp, KAC_VACBANNED, "I server protetti dall'anticheats Kigen Redux (KACR) non permettono l'ingresso ai giocatori bannati dall'anticheats della VALVE (VAC)");
	...
	SetTrieString(f_hTemp, KAC_MUTED, "%N è stato mutato dall'anticheats Kigen Redux");
	...
	SetTrieString(f_hTemp, KAC_UNABLETOCONTACT, "Impossibile contattare il KACR Master");
	...
	SetTrieString(f_hTemp, KAC_AUTOASSIGNED, "[KACR] Sei stato assegnato forzatamente ad un Team");
	SetTrieString(f_hTemp, KAC_SAYBLOCK, "[KACR] Il tuo testo è stato bloccato a causa di alcuni caratteri non validi");
	SetTrieString(f_hTemp, KAC_FORCEDREVAL, "[KACR] Convalida forzata di tutti i giocatori connessi");
	SetTrieString(f_hTemp, KAC_CANNOTREVAL, "[KACR] Non si può forzare la validazione dei giocatori finchè questi non siano stati tutti validati");
}